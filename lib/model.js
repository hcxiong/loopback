...
Model._getRemotingIdType = function getIdType(ModelCtor) {
  var idName = ModelCtor.definition.idName();
  var idProp = ModelCtor.definition.properties[idName];
  return getRemotingType(idProp);
};

Model._getRemotingFkType = function getForeignKeyType(ModelCtor, relationName) {
  var relation = ModelCtor.relations[relationName];
  var prop;
  if (relation.type === 'belongsTo') {
    prop = relation.modelFrom.definition.properties[relation.keyFrom];
  } else if (relation.modelThrough) {
    prop = relation.modelThrough.definition.properties[relation.keyTo];
  } else {
    prop = relation.modelTo.definition.properties[relation.keyTo];
  }
  return getRemotingType(prop);
};
/*!
...
Model.setup = function() {
  ...
  var idType = ModelCtor._getRemotingIdType(ModelCtor);
  ModelCtor.sharedCtor.accepts = [
    {arg: 'id', type: idType, required: true,
      http: {source: 'path'}, description: idDesc}
...
Model.nestRemoting = function(relationName, options, cb) {
  ...
  if (relation && relation.modelTo && relation.modelTo.sharedClass) {
    ...
    if (relation.multiple) {
      ...
      acceptArgs = [
        {
          arg: paramName, type: self._getRemotingFkType(self, relationName),
          http: { source: 'path' },
